...
## 5. ✅ DOs and DONTs: Lessons from Production Deployments

### ✅ DO: Understand the File Structure

**CRITICAL**: The `update.sh` script copies **ONLY** what's inside your app folder to the server.

**Example:**
```text
Dashboard_Puertos/              ← This folder gets copied
├── index.html                  ✅ Deployed to /var/www/html/petral/
├── script.js                   ✅ Deployed
├── style.css                   ✅ Deployed
└── Suply.Chain.Iron.Ore/       ✅ Deployed as subfolder
    └── ubicacion_fajas_map.html
```

**What DOESN'T get deployed:**
```text
Petral.MARK/                    ← Root folder (NOT copied)
├── Dashboard_Puertos/          ← Only THIS gets copied
├── Suply.Chain.Iron.Ore/       ❌ NOT deployed (outside Dashboard_Puertos)
└── audio_transcrip/            ❌ NOT deployed
```

**Rule**: If you want a file/folder accessible via `geeksoft.tech/petral/yourfile.html`, it **MUST** be inside `Dashboard_Puertos/`.
...
1. **Check File Paths**:
   ...
   Get-ChildItem -Recurse Dashboard_Puertos/ | Select-String -Pattern "href=|src="
   ```

2. **Test JavaScript Functions**:
   - Open browser DevTools (F12)
   - Check Console for errors
   - Test all buttons/checkboxes manually

3. **Verify Relative Paths**:
   ```html
   <!-- ✅ GOOD: Relative path -->
   <a href="Suply.Chain.Iron.Ore/ubicacion_fajas_map.html">Map</a>
   
   <!-- ❌ BAD: Absolute path (breaks on server) -->
   <a href="/Suply.Chain.Iron.Ore/ubicacion_fajas_map.html">Map</a>
   ```

---

### ❌ DON'T: Modify Files in the Root and Expect Them to Deploy

**Common Mistake:**
```text
# You edit this file:
C:\Users\rguti\Petral.MARK\index.html

# But update.sh copies from:
C:\Users\rguti\Petral.MARK\Dashboard_Puertos\index.html

# Result: Your changes DON'T appear on the website! ❌
```

**Solution**: Always edit files **inside** the app folder (`Dashboard_Puertos/`).

---

### ❌ DON'T: Forget to Define JavaScript Functions

**Symptom**: Button does nothing, Console shows `Uncaught ReferenceError: functionName is not defined`

**Example from Today's Bug:**
```html
<!-- HTML calls this function -->
<input type="checkbox" onchange="toggleCalculators()">
```

```javascript
// ❌ BAD: Function doesn't exist in script.js
// (Nothing happens when checkbox is clicked)

// ✅ GOOD: Function is defined
...
**Prevention**: After adding HTML with `onclick` or `onchange`, **immediately** add the corresponding function to `script.js`.

---

### ✅ DO: Clear Browser Cache After Deployment

**Why**: Browsers cache CSS/JS files aggressively. Even after successful deployment, you might see the old version.
...
3. **Incognito Mode**: `Ctrl + Shift + N` (Chrome) — Always loads fresh files

**Pro Tip**: Add version numbers to file references:
```html
<script src="script.js?v=2.7"></script>  <!-- Change v=2.7 to v=2.8 after updates -->
```

---

### ❌ DON'T: Push Large Files (Audio/Video) to Git

**Problem**: Git is designed for code, not media. Large files:
- Slow down `git push` and `git pull`
- Bloat repository size permanently (even after deletion)
- Can exceed GitHub's 100MB file limit
...
2. **Use External Storage**: Upload media to cloud storage (AWS S3, Google Drive, etc.) and link to it.

3. **Selective Commits**:
   ```powershell
   # ✅ GOOD: Only add specific files
   git add Dashboard_Puertos/script.js Dashboard_Puertos/index.html
   
   # ❌ BAD: Adds everything (including videos)
   git add .
   ```

---

### ✅ DO: Verify Deployment on Server

**After running `update.sh`, always check:**

```bash
# 1. Verify files were copied
ls -lh /var/www/html/petral/

# 2. Check specific file exists
cat /var/www/html/petral/index.html | grep "FUNCIONES"

# 3. Test a specific feature
curl http://geeksoft.tech/petral/Suply.Chain.Iron.Ore/ubicacion_fajas_map.html
```

**If you see 404**: The file wasn't deployed. Check:
1. Is it inside `Dashboard_Puertos/`?
2. Did you commit and push it?
3. Did you run `update.sh`?

---

### ✅ DO: Use Descriptive Commit Messages

**Why**: When debugging, you need to know what changed and when.

```bash
# ✅ GOOD: Clear, specific
git commit -m "Fix: Add toggleCalculators function and deploy Supply Chain Map folder"
...
Example: "Fix: Calculator visibility toggle not working"
```
...
**Temptation**: SSH into server and edit `/var/www/html/petral/index.html` directly.

**Problem**: Next time you run `update.sh`, your changes are **overwritten** by Git.

**Correct Workflow**:
1. Edit locally: `C:\Users\rguti\Petral.MARK\Dashboard_Puertos\index.html`
2. Test locally
3. Commit and push to Git
4. Run `update.sh` on server

**Exception**: Quick debugging (view logs, check file contents). But **never** make permanent changes on the server.

---

### ✅ DO: Keep a Deployment Checklist

**Before Every Deploy:**
- [ ] Tested locally (all buttons, links, functions work)
- [ ] Checked browser Console for JavaScript errors
- [ ] Verified file paths are correct
- [ ] Committed with descriptive message
- [ ] Pushed to `deploy-vps-2026.02.04.18.10` branch
- [ ] Ran `update.sh` on server
- [ ] Cleared browser cache and tested live site
- [ ] Checked server logs if something doesn't work

---

## 6. Quick Reference: Common Deployment Errors

| Symptom | Cause | Fix |
|---------|-------|-----|
| **404 Not Found** | File not in `Dashboard_Puertos/` | Move file to correct folder, commit, push, deploy |
| **Button does nothing** | JavaScript function not defined | Add function to `script.js` |
| **Old version showing** | Browser cache | Hard reload (`Ctrl + Shift + R`) |
| **Changes not appearing** | Edited wrong file (root instead of app folder) | Edit file inside `Dashboard_Puertos/` |
| **Git push very slow** | Large files (audio/video) in commit | Add to `.gitignore`, use selective commits |
